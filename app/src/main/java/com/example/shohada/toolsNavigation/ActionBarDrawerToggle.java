 ...
 */
package com.example.shohada.toolsNavigation;
...
public class ActionBarDrawerToggle implements DrawerLayout.DrawerListener {
    ...
    {
        boolean isOpen = mDrawerLayout.isDrawerOpen(GravityCompat.END);
        if (isOpen)
        {
            setPosition(1);
        }
        ...
        if (mDrawerIndicatorEnabled)
        {
            setActionBarUpIndicator((Drawable) mSlider, mDrawerLayout.isDrawerOpen(GravityCompat.END) ? mCloseDrawerContentDescRes : mOpenDrawerContentDescRes);
        }
    }
